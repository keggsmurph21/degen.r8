...
function commitToPot(pot, ps, balance) {
    ...
    if (maxCommitment === 0)
        return 0;
    var marginalContribution = Math.min(maxCommitment, balance);
    var cumulativeContribution = pot.contributions[index] + marginalContribution;
...
var Round = (function () {
    ...
    Round.prototype.filterPots = function () {
        ...
        this.pots = this.pots.filter(function (pot) {
            ...
            playerToRefund.player.balance += pot.contributions.reduce(function (acc, contribution) { return acc += contribution; }, 0);
            return false;
        });
