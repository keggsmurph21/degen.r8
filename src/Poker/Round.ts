...
export class Round {
    ...
    constructor(players: Player[], params: RoundParameters) {
        players.forEach((player, i) => {
            ...
            if (params.useAntes) {
                player.decrementBalance(params.anteBet); // NB: Doesn't affect current bet
    ...
    public makeBet(player: Player, bet: Bet, requestedRaiseAmount: number = 0): void {
        if (this.isFinished)
        ...
        const callAmount = this.currentBet - currentPlayerState.amountBetThisRound;
        ...
        case Bet.Call:
            ...
            break;
        case Bet.Raise:
            const raiseAmount = requestedRaiseAmount - callAmount; // Actual amount we're "raising" by
            if (raiseAmount <= 0)
        ...
        case Bet.Fold:
            ...
            const remaining = this.playerStates.map(playerState => {
