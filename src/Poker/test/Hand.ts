...
import {getBestFiveCardHand, HandType, SevenCards} from "../Hand";

const expectHand =
    (cards: SevenCards, handType: HandType, highestRanks: Rank[] = []) => {
        const bestHand = getBestFiveCardHand(cards);
        expect(bestHand.type).to.equal(handType);
        highestRanks.forEach(
            (rank, i) => { expect(bestHand.ranks[4 - i]).to.equal(rank); });
    };
